...
! ==========================================================================================================
      ...
      ELSE ! NONZERO HYPERVISCOSITY
        PSIVIS = HELMP(PSI, FINFO%VISCPOW, 0.D0, FINFO%VISC/(FINFO%VISCP*(-1)**(FINFO%VISCPOW/2+1)))
        PSIVIS%E = PSIVIS%E * (FINFO%VISCP*(-1)**(FINFO%VISCPOW/2+1))
        CHIVIS = HELMP(CHI, FINFO%VISCPOW, 0.D0, FINFO%VISC/(FINFO%VISCP*(-1)**(FINFO%VISCPOW/2+1)))
        CHIVIS%E = CHIVIS%E * (FINFO%VISCP*(-1)**(FINFO%VISCPOW/2+1))
      ENDIF
...
! ==========================================================================================================
      ...
      ELSE ! F(K+1) = (1-DT*VISC*DEL2-DT*VISCP*DELP)^(-1) F(K+1/2)
        ...
        A = -1.D0/(DT*(FINFO%VISCP*(-1)**(FINFO%VISCPOW/2+1)))
        B = FINFO%VISC/(FINFO%VISCP*(-1)**(FINFO%VISCPOW/2+1))
        PSIH%LN = A*PSIH%LN
        PSIH%E = A*PSIH%E
...
! ==========================================================================================================
      ...
      ELSE IF (ORDER_ .EQ. 2) THEN
        ...
        ELSE ! F(K+1) = (1-DT/2*VISC*DEL2-DT/2*VISCP*DELP)^(-1) [F(K+1/2) + DT/2*LTERM(K)]
          ...
          A = -2.D0/(DT*(FINFO%VISCP*(-1)**(FINFO%VISCPOW/2+1)))
          B = FINFO%VISC/(FINFO%VISCP*(-1)**(FINFO%VISCPOW/2+1))
          PSIH%LN = A*(PSIH%LN + DT/2.D0*PSIVISP%LN)
          PSIH%E = A*(PSIH%E + DT/2.D0*PSIVISP%E)
          PSI = IHELMP(PSIH, FINFO%VISCPOW, ALP=A, BET=B, ISTP=.TRUE.)
